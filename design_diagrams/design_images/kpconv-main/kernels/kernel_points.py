...
def load_kernels(radius, num_kpoints, dimension, fixed, lloyd=False):
    ...
    # Kernel_file
    kernel_file = join(kernel_dir, 'k_{:03d}_{:s}_{:d}D.las'.format(num_kpoints, fixed, dimension))
    ...
    if not exists(kernel_file):
        ...
        write_las(kernel_points,None,None,kernel_file)

    else:
        data = read_file_auto(kernel_file)
        kernel_points = data["coordinates"]
