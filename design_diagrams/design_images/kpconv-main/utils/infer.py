...
class ModelTester:
    ...
    def cloud_segmentation_test(self, net, test_loader, config, num_votes=100, debug=False):
        ...
        # Test saving path
        if config.saving:
            csp= config.saving_path.replace("\\", "/").split('/')[-1]
            test_path = join(config.path_output, csp)
        ...
        while True:
            ...
            print('Test epoch {:d}, end. Min potential = {:.4f}'.format(test_epoch, new_min))
            ...
            if last_min + 1 < new_min:
                ...
                if int(np.ceil(new_min)) % 10 == 0:
                    ...
                    for i, file_path in enumerate(test_loader.dataset.files):
                        ...
                        preds = test_loader.dataset.label_values[np.argmax(proj_probs[i], axis=1)].astype(np.int32)
                        #-------------------------------------------------
                        cloud_name = file_path.replace("\\", "/").split('/')[-1]
                        test_name = join(test_path, 'predictions', cloud_name[:-4] + '.las')
                        ...
                        write_las(pot_points.astype(np.float32),None,pots,pot_name)
