...
def find_max_epoch_in_saved_checkpoints(save_path,second=False):
    # Regex pattern to match filenames like 'model_XX_checkpoint.pth'
    if second:
            pattern = re.compile(r'model2_(\d+)_checkpoint\.pth')
    else:
        pattern = re.compile(r'model_(\d+)_checkpoint\.pth')
...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=5, help='batchsize in training')
...
def main2(args, logger):
    ...
    # Find the maximum completed epoch in the save path
    max_completed_epoch = find_max_epoch_in_saved_checkpoints(args.save_path)
    if max_completed_epoch>0:
        logger.info(f"########### Start at {max_completed_epoch} ###############")
        model.load_state_dict(torch.load(os.path.join(args.save_path, 'model_' + str(max_completed_epoch) + '_checkpoint.pth')))

    is_Growing = False
    for epoch in range(max_completed_epoch + 1, args.max_epoch[0] + 1):
    ...
    for epoch in range( 1, args.max_epoch[1] + 1):
        ...
        if epoch % 10 == 0:
            torch.save(model.state_dict(), join(args.save_path,  'model2_' + str(epoch) + '_checkpoint.pth'))
            torch.save(classifier.state_dict(), join(args.save_path, 'cls2_' + str(epoch) + '_checkpoint.pth'))
            with torch.no_grad():
                o_Acc, m_Acc, s = eval(epoch, args, test_areas,second_round=True)
                logger.info('Epoch: {:02d}, oAcc {:.2f}  mAcc {:.2f} IoUs'.format(epoch, o_Acc, m_Acc) + s)
...
def main(args, logger):
    ...
    is_Growing = False
        # Find the maximum completed epoch in the save path
    start_grow_epoch = find_max_epoch_in_saved_checkpoints(args.save_path)
    if start_grow_epoch>0:
        logger.info(f"########### Start at {start_grow_epoch} ###############")
        model.load_state_dict(torch.load(os.path.join(args.save_path, 'model_' + str(start_grow_epoch) + '_checkpoint.pth')))

    for epoch in range(start_grow_epoch+1, args.max_epoch[0] + 1):
        '''Take 10 epochs as a round'''
        if (epoch - 1) % 10 == 0:
            classifier = cluster(args, logger, cluster_loader, model, epoch, start_grow_epoch, is_Growing)
        train(train_loader, logger, model, optimizer, loss, epoch, scheduler, classifier)
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum, dampening=args.dampening, weight_decay=args.weight_decay)
    scheduler = PolyLR(optimizer, max_iter=args.max_iter[1])
    for epoch in range(start_grow_epoch+1, args.max_epoch[1] + 1):
        '''Take 10 epochs as a round'''
        if (epoch - 1) % 10 == 0:
            classifier = cluster(args, logger, cluster_loader, model, epoch, start_grow_epoch, is_Growing)
        train(train_loader, logger, model, optimizer, loss, epoch, scheduler, classifier)
...
def cluster(args, logger, cluster_loader, model, epoch, start_grow_epoch=None, is_Growing=False):
    ...
    primitive_labels = KMeans(n_clusters=args.primitive_num, n_init=5, random_state=0).fit_predict(sp_feats.numpy())
